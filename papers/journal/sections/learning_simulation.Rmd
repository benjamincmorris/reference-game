...
In our simulations, we varied the child's learning rate ($p$) from .1 to 1 in steps of .1 as in the Teaching simulation, the parents' future-weighting ($\lambda$) from .1 to 1 in steps of .1, the parents' rationality ($\alpha$) from .5 to 3 in steps of .5, and considered three values each of the cost of speaking ($S = (0, 10, 20)$) and pointing ($P = (50, 60, 70)$). The utility of communicating successfully was always 100.
...
In order to understand how learning rates vary with model parameters, we first discuss the dependence of each of the three tested models on it's parameters, and then discuss relationships between the models. For clarity of exposition, we analyze the number of events required for `r THRESHOLD*100`% of simulated learners to acquire the target word, and plot a representative subset of parameter values. 
