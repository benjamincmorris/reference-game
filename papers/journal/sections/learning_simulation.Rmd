...
com_time_to_90 <- com_props %>%
  group_by(alpha, lambda, p, P, S) %>%
  filter(prob >= THRESHOLD) %>%
  slice(1)

ggplot(com_time_to_90 %>%
         filter(alpha %in% c(.5,  2),
              # p %in% c(.2, .8),
                lambda %in% c(.2, .8)),
       aes(x = p, y = trial, color = as.factor(alpha), 
...
teach_time_to_90 <- teach_props %>%
  group_by(p) %>%
  filter(prob >= THRESHOLD) %>%
  slice(1) %>%
  mutate(model = "teach")

selected_com_to_90 <- com_time_to_90 %>%
  ungroup() %>%
  filter((P == 50 & S == 20) | (P == 70 & S == 0)) %>%
  group_by(S, P, p) %>%
  summarise(min = min(trial), max = max(trial)) %>%
  left_join(com_time_to_90 %>%
              filter((P == 50 & S == 20) | (P == 70 & S == 0),
                     alpha == 1.5, lambda == .4) %>%
              group_by(S, P, p) %>%
...
selected_xsit_to_90 <- xsit_time_to_90 %>%
  ungroup() %>%
  filter(M == 128, C > 1) %>%
  mutate(model = "talk") %>%
...
plot_com <- selected_xsit_to_90 %>%
  distinct(C, nguesses, p) %>%
  left_join(selected_com_to_90, by = "p") %>%
  mutate(costs = factor(costs, labels = c("Higher speech efficiency",
                                                  "Lower speech efficiency")))

plot_xsit <- selected_com_to_90 %>%
  distinct(costs, p) %>%
  left_join(selected_xsit_to_90, by = "p") %>%
  mutate(costs = factor(costs, labels = c("Higher speech efficiency",
                                                  "Lower speech efficiency")))
```
...
xsit_teach_ratio <-xsit_time_to_90 %>% 
  filter(M == 128, C > 1) %>%
  group_by(p) %>% 
  summarise(min_trial = min(trial),
            max_trial = max(trial)) %>%
...
Because the real-world parameters appropriate for each model are difficult to determine, we consider the relationship between the models over the range of possible parameters. Figure \ref{fig:compare-models} shows the time for 90% of learning to acquire a word in each of the three models. Across all possible child learning rates ($p$), the Teach model lead to the fastest learning as expected. We can treat this model as a lower bound how quickly learning could possibly happen. 

For the Communication model, we considered the range of all possible rates of learning that could unfold as the parameters of both child and parent varied. The range was substantial. If parents weight the future near equally to the present, and are highly rational, the child's resultant rate of learning is nearly identical to the rate of learning under the Teach model: Children required `r com_teach_ratio$min_ratio` times as many learning instances under the Communicate model as the Teach model. In contrast, if the parent weighs the future much less than the present, and is relatively irrational about maximizing utility, the rate of learning can be quite slow--in the worst case requiring children to have `r com_teach_ratio$min_ratio` as many learning instances as under the Teach model. Despite this bad worst case scenario, if parents' parameters are close to the ones we estimated in our experiment, Communicate would require only `r empirical_teach_ratio$min_ratio` as many instances as Teach if speech is high efficiency relative to pointing, and `r empirical_teach_ratio$max_ratio` as many instances if speech is lower efficiency. 

For the Talk model, we also observed a wide range of times to learn as a function of both the ambiguity of the learning environment defined by the number of objects that could be candidate referents ($C$) and the number of simultaneous hypotheses that the child could maintain. When the environment was unambiguous--only 2 objects were present at a time, and the child could encode both, learning under Talk was almost identical to learning under Teach (requiring `r xsit_teach_ratio$min_ratio` times as many instances). In contrast, if ambiguity was high and learners could only track a single hypothesis, learning was significantly slower under Talk than teach, (requiring `r xsit_teach_ratio$max_ratio` times as many instances).

Comparing Communicate and Talk to each-other, we find that that Talk wins out unambiguously when context is low in ambiguity and learners can encode multiple hypotheses. However, as context grows in ambiguity, Communicate outpaces Teach--especially when learners can store only one or a few hypotheses. 

```{r compare-models, fig.width = 6, fig.height = 4}
ggplot(plot_xsit, aes(x = p)) +
  ...
  geom_ribbon(aes(ymin = min, ymax = max), alpha = .15,
              data = plot_com) +
  geom_line(aes(y = trial), data = teach_time_to_90) +
