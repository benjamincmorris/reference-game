...
CONDITION_LABELS <- c("Pointing relatively cheap", "Pointing relatively costly")
```
...
prop_methods_exposures <- filtered_data %>%
  group_by(condition, ldf_num, partnersExposure, exposureRate, method) %>%
  summarise(n = n()) %>%
  mutate(n = n / sum(n)) %>%
  ungroup() %>%
...
ggplot(prop_methods_exposures,
       aes(x = partnersExposure, y = empirical_stat, color = method, 
           label = method, group = interaction(method, condition))) +
...
```{r teaching-rates}
glm_teach <- glmer((method == "teach") ~ partnersExposureNumeric * exposureRate + 
                    partnersExposureNumeric * appearanceNumeric +
                    condition + 
                    (1| ldf_num) + (1|realLabel),
            data = filtered_data,
...
plot_prop_methods_teaching %>%
  ...
  geom_linerange(aes(ymax = ci_upper, ymin = ci_lower, color=partnersExposure), position=position_dodge(.25)) +
