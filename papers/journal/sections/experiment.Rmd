...
This sample size, experimental design, and analysis plan were pre-registered at https://osf.io/63qdg. Sample size was determined based on prior pilot experiments. All data, analysis code, and research materials are available at https://osf.io/d9gkw/. Data were analyzed using \texttt{R} version 4.1.1 [@r-core-team2021]. Models were estimated using version 1.1-27.1 of the \texttt{lme4} package [@bates2015]. 
...
ggplot(acc_data, aes(x = as.factor(exposureRate), y = empirical_stat, 
  ...
  xlab("Paticipant's training exposure") +
...
Participants pointing behavior in this game is not just reflecting their own knowledge or the general efficiency of pointing, but is crucially modulated by their beliefs about their partner's knowledge. These patterns mirror previous corpus analyses demonstrating parents' use of pointing in naturalistic parental communicative behaviors [see @yurovsky2018children]. Note that these effects cannot be explained by solely participants' knowledge; all patterns above hold when looking *only* at words known by the participant at pretest (*ps < 0.01*).
