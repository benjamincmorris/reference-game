...
Do derive predictions from the model, we ran 100 simulations of the model's choices participant by participant and trial by trial using our posterior estimates of the hyper-parameters $\alpha$ and $\gamma$. Because we did not use our participant-level parameter estimates, this reduces the correlation between predictions and empirical data, but reflects the model's best predictions about a the results of a replication of our experiment. Figure \ref{fig:model-predictions}a shows the predictions from the model in analogous format to the empirical data in Figure \ref{fig:modalities}. The model correctly captures the qualitative trends in participants' behavior: It speaks more and points less in the Higher speech efficiency condition, and is more likely to teach less knowledgeable partners. Figure \ref{fig:model-predictions}b shows the model's predicted teaching behavior in an analogous format to the empirical data in Figure \ref{fig:teach-empirical}. Here the model again captures the qualitative trends apparent in participants' behavior. The model teaches less knowledgeable partners, especially those who it believes have no language knowledge at all. The model teaches more when speech is a more efficient modality, and thus the future utility of teach a partner is higher. And finally the model teaches most on the first occurrence of each object, and becomes less likely to teach on future occurrences when (1) partners should be more likely to know object labels, and (2) the expected future rewards of teaching are smaller.
...
```{r model-predictions, fig.height = 5.5, fig.width = 4.25, set.cap.width=T, num.cols.cap=1, fig.cap = "(a) Model prediction choice of communicative method choice as a function of exposure and the utility manipulation. (b) Model predicted probabiliyt of teaching by Partner's language knowledge and exposure rate."}
...
```{r model-fit, fig.env = "figure", fig.pos = "H", fig.height=3,  fig.width=4.5, fig.cap = "Fit between model predictions and empirical data."}
