...
```{r model-predictions, fig.height = 3, fig.width = 4.25, set.cap.width=T, num.cols.cap=1, fig.cap = "Speaker communicative method choice as a function of exposure and the utility manipulation."}
...
modality_plot(all_data_model, action, model_label_data)
```
...
predicted_data <- map_dfr(predicted_files, read_csv, .id = "sample")
```
...
get_model_output <- function(sample_data) {
  ...
  sample_data %>%
    select(condition,ldf_num, partnersExposure, appearance, exposures, action) %>%
...
model_wide_data <- predicted_data %>%
  group_by(sample) %>%
  nest() %>%
...
How well did our model characterize people's communicative behavior? We ran 100 simulations of the model's choices participant by participant and trial by trial using our posterior estimates of the hyper-parameters $\alpha$ and $\gamma$, Because we did not use our participant-level parameter estimates, this reduces the correlation between predictions and empirical data, but reflects best the best predictions about what a replication experiment. Across experimental manipulations, the model's predictions were highly correlated with participant behavior ($r =$ `r pull(model_human_correlation, estimate)` [`r pull(model_human_correlation, conf.low)`, `r pull(model_human_correlation, conf.high)`], $t($ `r pull(model_human_correlation, parameter)` $)=$ `r pull(model_human_correlation, statistic)`, $p$ `r pull(model_human_correlation, p.value)`; Figure \ref{fig:model-fit}).


```{r model-fit, fig.env = "figure", fig.pos = "H", fig.height=3,  fig.width=4, fig.cap = "Fit between model predictions and empirical data."}
