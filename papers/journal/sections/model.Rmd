...
Do derive predictions from the model, we ran 100 simulations of the model's choices participant by participant and trial by trial using our posterior estimates of the hyper-parameters $\alpha$ and $\gamma$. Because we did not use our participant-level parameter estimates, this reduces the correlation between predictions and empirical data, but reflects best the best predictions about a replication experiment. Figure \ref{fig:model-predictions} shows the predictions from the model in analogous format to the empirical data in Figure \ref{fig:modalities}. The model correctly captures the qualitative trends in participants' behavior: It speaks more and points less in the Higher speech efficiency condition, and is more likely to teach less knowledgeable partners.


Across experimental manipulations, the model's predictions were highly correlated with participant behavior ($r =$ `r pull(model_human_correlation, estimate)` [`r pull(model_human_correlation, conf.low)`, `r pull(model_human_correlation, conf.high)`], $t($ `r pull(model_human_correlation, parameter)` $)=$ `r pull(model_human_correlation, statistic)`, $p$ `r pull(model_human_correlation, p.value)`; Figure \ref{fig:model-fit}).
