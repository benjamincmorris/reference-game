...
Estimating these parameters also required us to give each a prior. The rationality parameter $\alpha$ controls the sensitivity of the model to differences in utility and can range over the non-negative real numbers $\left[0,\infty\right)$. A rationality value of 1 corresponds to probability matching--in which actions are chosen according to the @luce1959 Choice rule, and larger values correspond to increasingly great moves towards maximizing. For this chose a distribution of the prior with a mean of 1, but an Exponential shape with a long right tail: 
...
Importantly, because the model's parameters were estimated many trials of behavior from hundreds of participants, the priors are dwarfed by the likelihoods in estimating the parameters' posterior distributions. The estimates reported below are thus robust to many other prior choices.
