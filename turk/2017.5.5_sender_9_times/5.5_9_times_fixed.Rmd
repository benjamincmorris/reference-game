---
title: "5.5 Timer Starts at Key Press, 3 objects 9 Times FIXED"
...
prop_methods <- all_data %>%
  ...
  mutate(appearance = if_else(trialnum == min(trialnum), "First",
                              if_else(trialnum == max(trialnum), "Ninth", "Middle"))) %>%
...
anti_join(all_methods, select(prop_methods, partnersExposure, appearance, method)) %>%
  ...
  mutate(appearance = factor(appearance, levels = c("First", "Middle", "Ninth")),
