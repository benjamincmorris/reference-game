...
#Load packages + setup
library(rwebppl) #use dyurovsky/rwebppl
library(data.table)
library(here)
...
```{r read-datamodel, message = FALSE}
...
trial_level_data <- model_data %>%
  ...
  rename(partnersExposureDescriptive = partnersExposure) %>%
  mutate(partnersExposure = case_when(partnersExposureDescriptive == "None" ~ 0,
                                      partnersExposureDescriptive == "Same" ~ 1,
                                      partnersExposureDescriptive == "Perfect" ~ 2),
         method = case_when(method == "label" ~ "speak",
                            method == "click" ~ "point",
                            method == "label_click" ~ "teach"))
```
...
model_names = c("full_model", "no_planning", "no_cost", "no_success")

sample_data <- trial_level_data %>% filter(ldf_num %in% 301:480)

run_single_model <- function(data) {
  num <- data[[1,"num"]]
  print(num)
  output <- webppl(program_file = here("model/comparisons/fit_single_participant_nosuccess.wppl"),
                                      data = data, data_var = "empiricalData")
  write_csv(output, here(glue("model/samples/no_success/p{num}.csv")))
  return(output)
}
...
files <- list.files(here("model/samples/no_cost"), pattern = "*.csv", 
                    full.names = TRUE)
...
ggplot(mean_data %>% pivot_longer(cols = c(alpha, discount, score), 
                                  names_to = "Parameter"), 
       aes(x = value)) + 
  geom_histogram() + 
...
```{r write-parameters}
write_csv(hyper_parameters, here("model_output/no_success/hyper_parameters.csv"))
```

```{r read-hyperparameters}
hyper_parameters <- read_csv(here("model_output/no_success/hyper_parameters.csv"),
                             show_col_types = FALSE)
...
setup_data <- trial_level_data %>%
  ungroup() %>%
  mutate(alpha = mean_hyper_parameters %>% 
           filter(Parameter == "hyperAlpha") %>%
           pull(value),
         discount = mean_hyper_parameters %>% 
           filter(Parameter == "hyperDiscount") %>%
           pull(value)) 

predict_outcomes <- function(sample) {
  predicted_data <- setup_data %>%
    mutate(action = webppl(
      program_file = here("model/comparisons/run_model_nosuccess.wppl"), 
                         data = ., 
                         data_var = "empiricalVocabs") %>%
           bind_rows()) %>%
            mutate(likelihood = action[,2],
                   action = action[,1]) %>%
           as_tibble()

  write_csv(predicted_data,
            here(glue("model_output/no_success/predicted_data/predicted_data{sample}.csv")))
}

walk(1:2, predict_outcomes)
```

```{r load-model-predictions}
out_files <- list.files(here("model_output/no_cost/predicted_data/"), pattern = "*.csv", 
                    full.names = TRUE)
...
predicted_data <- map_dfr(out_files, read_output_file)
```

```{r}
no_planning_likelihood <- predicted_data %>%
  group_by(sample) %>%
  summarise(likelihood = sum(likelihood), alpha = mean(alpha), 
            discount = mean(discount)) %>%
  mutate(likelihood = likelihood + dnorm(logit(discount), log = TRUE) + 
           dgamma(alpha, 1, 1, log = TRUE)) %>%
