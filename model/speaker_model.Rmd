...
```{r write-parameters}
write_csv(hyper_parameters, here("model_output/hyper_parameters.csv"))
```
...
predicted_data <- trial_level_data %>%
  ...
  mutate(alpha = mean_hyper_parameters %>% 
         ...
         discount = mean_hyper_parameters %>% 
           ...
           pull(value)) %>%
  mutate(action = webppl(program_file = here("model/run_model.wppl"), 
                         data = ., 
...
write_csv(predicted_data, here("model_output/predicted_data.csv"))
```
