...
sender_test %>%
  mutate(testCorrect=ifelse(typedLabel=='UNKNOWN', "Unknown", as.character(responseCorrect))) %>%
...
(Note that in this figure, the partner's exposure condition is not properly ordered.)  <br>
...
label_props %>% 
  ...
  scale_fill_discrete(name="Partner's Exposure", labels=c("None", "Same", "Half")) +
  coord_cartesian(ylim=c(0,1))
...
```{r, echo=FALSE, warning=FALSE, message=FALSE, fig.width=5, fig.height=8}
seprop <- function(props) {
  mean_prop = mean(props)
  sqrt( (mean_prop*(1 - mean_prop)) / length(props))
}
...
anti_join(all_methods, select(prop_methods, partnersExposure, appearance, method)) %>%
  ...
  mutate(appearance = factor(appearance, levels = c("First", "Second", "Third")),
         ...
         partnersExposure = factor(partnersExposure, levels = c("0", "1/2", "1"))) %>%
  ...
  scale_fill_discrete(name = "Message Type", labels=c("Click", "Label", "Teach"))+
