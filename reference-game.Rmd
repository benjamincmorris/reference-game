...
filenames <- list.files("turk/12.8_reference_100_30/production-results/", pattern="*.json", full.names = TRUE)
...
for (i in 1:length(ldf)) {
  ...
  exposures <- rbind(exposures, cbind(subID, condition, janeDoe$answers$data$expDuration))
...
filenames <- list.files("turk/12.8_reference_80_50/production-results/", pattern="*.json", full.names = TRUE)
ldf <- lapply(filenames, fromJSON)
for (i in 1:length(ldf)) {
  janeDoe <- ldf[[i]]
  # anyAttn <- ifelse(length(janeDoe$answers$data$attnCheck)!=0, 1,0)
...
filenames <- list.files("turk/1.5_reference_receiver_pilot/production-results/", pattern="*.json", full.names = TRUE)
ldf <- lapply(filenames, fromJSON)
res <- lapply(ldf, summary)
...
for (i in 1:length(ldf)) {
  ...
  # subID <- i
  timing <- rbind(timing, as.data.frame(
      cbind(start= janeDoe$AcceptTime,stop = janeDoe$SubmitTime)))
  exposures <- rbind(exposures, cbind(condition, janeDoe$answers$data$expDuration))
  ruleCheck <- rbind(ruleCheck, cbind(condition, janeDoe$answers$data$ruleQuestions))
  test <- rbind(test, cbind(condition, janeDoe$answers$data$testTrials))
  game <- rbind(game,cbind(condition, janeDoe$answers$data$gameTrials))
  if (length(janeDoe$answers$data$attnCheck)!=0) 
    {attn <- rbind(attn, cbind(condition, janeDoe$answers$data$attnCheck))}
}

test<- left_join(test, exposures %>% select(subID, object, realLabel) %>% distinct(), 
...
game<- left_join(game, exposures %>% select(subID, object, realLabel, exposureRate) %>% distinct(), 
                by=c('targetObjectName'='object', "subID")) %>%
...
game %>%
  group_by(exposureRate,receivedMessageType, subID) %>%
...
game2 <- game %>%
  ...
  mutate(gameCorrect = ifelse(gameClosest==targetLabel, 1, 0),
         gameCorrect = if_else(typedLabel == "", 1, gameCorrect),
         gameClosest = if_else(typedLabel == "", targetLabel, gameClosest)) %>%
  mutate(receiverCorrect = ifelse(gameClosest==clickedLabel, 1, 0)) %>%
  left_join(rename(select(test,condition,exposureRate,subID,realLabel,testCorrect),
                   gameClosest = realLabel))

game_responses <- game2 %>%
...
game %>% filter(subID == 10) %>% select( subID, exposureRate, realLabel, typedLabel, adjLabel)
