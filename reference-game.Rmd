...
filenames <- list.files("turk/2.20_sender/production-results/", pattern="*.json", full.names = TRUE)
...
for (i in 1:length(ldf)) {
  ...
  sender_test <- rbind(sender_test,janeDoe$answers$data$testTrials)
  sender_game <- rbind(sender_game,janeDoe$answers$data$gameTrials)
  if (length(janeDoe$answers$data$attnCheck)!=0) 
...
sender_game_fixed <- sender_game %>% 
  mutate(subID = as.numeric(subID)) %>%
  mutate(pExposure_fixed= ifelse(subID<=60, 0, partnersExposure)) %>% 
  mutate(pExposure_fixed= ifelse(subID<=50, "1/2", pExposure_fixed)) %>% 
  mutate(pExposure_fixed= ifelse(subID<=40, 1, pExposure_fixed)) %>% 
  mutate(pExposure_fixed= ifelse(subID<=30, 0, pExposure_fixed)) %>% 
  mutate(pExposure_fixed= ifelse(subID<=20, "1/2", pExposure_fixed))

tmp <- sender_game_fixed %>% distinct(subID, username)
which(duplicated(tmp$subID))
which(tmp$subID == 2)
which(tmp$subID == 31)
  # row nums are 17, 53, 3, 51

sender_game %>% filter(subID == 2)
sender_game_fixed %>% filter(subID == 31) %>% 
  select(subID, username, condition, pExposure_fixed, trialnum, realLabel, exposureRate, 
         method, typedLabel, clickedObject, responseCorrect)

sender_teaching <- sender_game_fixed %>% 
  filter(method == "label_click") %>% 
  select(subID, username, condition, pExposure_fixed, trialnum, realLabel, exposureRate, 
         method, typedLabel, clickedObject, responseCorrect) %>% 
  filter(pExposure_fixed == 1) %>%
  mutate(trialnum_prop = (trialnum - 36)/27)

nrow(sender_teaching)
nrow(sender_game_fixed)
...
tmp2 %>% filter(V1 %in% ldf[[53]]$WorkerId)
...
howLong %>% arrange(start)
sender_ruleCheck[which(howLong$howLong > as.duration(15*60)),] %>%
  select(subID)

tmp <- sender_game %>% distinct(subID, username) %>% select(subID)
cbind(howLong, tmp, sender_turkIds) %>% arrange(start) %>% filter(sender_turkIds %in% quit_participants)

tmp2 %>% mutate(actualPs = ifelse(tmp2$V1 %in% quit_participants, 1 , 0))
