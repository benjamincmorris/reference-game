...
filenames1 <- list.files("turk/2.20_sender/production-results/", pattern="*.json", full.names = TRUE)
filenames2 <- list.files("turk/2.21_sender_fix/production-results/", pattern="*.json", full.names = TRUE)
...
## DO THIS MANUALLY, each time check for any red flags (slidenums > 2)
sender_turkIds_clean <- as.vector(sender_turkIds)
# sender_turkIds_clean[which(sender_turkIds_clean %in% tmp)]
...
quit_participants <- sender_turkIds_clean[which(sender_turkIds_clean %in% tmp2$V1)]
tmp2 %>% filter(V1 %in% quit_participants)
#above is broken for 2.20, 2.21 data as is



#need to clean the timing data
howLong <- sender_timing %>% 
  mutate(start = ymd_hms(start), 
         stop = ymd_hms(stop),
         howLong = as.duration(ymd_hms(sender_timing$start) %--% ymd_hms(sender_timing$stop))) 
hist(howLong$howLong)

subIDs <- sender_game %>% distinct(subID, username)
orderedStart <- cbind(howLong, subIDs, sender_turkIds) %>% arrange(start) %>% mutate(subID =as.numeric(subID)) 

# BIG issue with 2.20 data where pExposure conditions were overwritten
# so we need to do some pruning
tmp <- sender_game %>% distinct(subID, username)
tmp <- sender_game %>% mutate(subID =as.numeric(subID)) %>%
                                   mutate(toBeDropped = ifelse(10<subID & subID<=30 & partnersExposure == "1", 1, 0)) %>%
  mutate(toBeDropped = ifelse(40<subID & subID<=60 & partnersExposure == "1", 1, toBeDropped))


## now we need to drop the duplicate subIDs, whoever was later to the party
all_data <- inner_join(tmp  %>% mutate(ldf_num = as.factor(ldf_num)), orderedStart)
all_data <- all_data %>% arrange(start)
folks_all <- all_data %>% distinct(subID, username, .keep_all=TRUE)
folks_dropped <- folks_all %>% filter(toBeDropped == 0) 
duplicates <- folks_dropped[duplicated(folks_dropped$subID),]
#need to be sure that we only grab unique attempts (e.g. username could be consistent across 2 attempts/subjects)
all_data <- all_data %>% 
  mutate(toBeDropped = ifelse(username %in% duplicates$username & start %in% duplicates$start, 1, toBeDropped))
## need to check for duplicated turkids too, only accept 1st attempt. 
## already sorted by start time, so just grab the results of duplicated
#repeatedTurkers <- folks_all[duplicated(folks_all$sender_turkIds),]
...
all_data <- all_data %>% 
    mutate(toBeDropped = ifelse(ldf_num %in% repeatedTurkers$ldf_num, 1, toBeDropped))
...
all_data_fixed <- all_data %>% select(-phase) %>% 
  left_join(sender_test_slim %>% mutate(ldf_num = as.factor(ldf_num))) 
...
all_data_dropped %>% 
  group_by(method) %>% 
  select(subID, username, condition, partnersExposure, trialnum, realLabel, exposureRate,
         typedLabel, clickedObject, responseCorrect) %>% 
  summarize(n=n())
...
all_data_dropped %>% 
  ...
  summarize(mean = mean(method=="label"), sd = sd(method=="label"), n=n()) %>%
  ggplot(aes(x=trialnum, y=mean, color=partnersExposure)) +
  geom_pointrange(aes(x= trialnum, y=mean, ymin=mean-sd, ymax=mean+sd), position=position_dodge(.75))
    
   
#first, second, or third appearence?
all_data_dropped %>%
  ...
  filter((subID %in% teach_subs$subID)) %>%
  group_by(appearance,partnersExposure, method, subID) %>% 
  summarize(mean = ifelse(n()>0, n()/27, 0)) %>%
  summarize_each(funs(mean, seprop, n()), mean) %>% 
  ggplot(aes(x=appearance, y=mean,  shape=partnersExposure, color=method)) +
  geom_pointrange(aes(x= appearance, y=mean, ymin=mean-seprop, ymax=mean+seprop), position=position_dodge(.75))
...
exposureData <- exposures %>% group_by(subID, exposureRate) %>% summarize(duration = sum(duration)) %>% 
...
howLong <- sender_timing %>% mutate(howLong = as.duration(ymd_hms(sender_timing$start) %--% ymd_hms(sender_timing$stop))) %>%
  select(subID, howLong)
sort(howLong$howLong)
mean(howLong$howLong)/60
